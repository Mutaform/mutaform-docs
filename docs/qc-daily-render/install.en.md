# Installation — QC Daily Render

Set it up once; after that Blender offers updates on its own.

## What you need

Blender **5.1.2** or newer. The version is in the window title, and in
`Help → About Blender`.

!!! warning "If you have Mutaform Studio Render installed"
    That is the same add-on under its old name. Version 1.6.0 changed the
    extension id, so Blender sees a **different** extension and will not offer
    an update.

    Remove the old copy first: `Edit → Preferences → Get Extensions`, find
    **Mutaform Studio Render**, the `∨` arrow on the right → **Uninstall**. Then
...
5. Paste this into the **URL** field:
    ...
    ```text
    https://mutaform.github.io/qc-daily-render/index.json
    ```
...
The panel appears in the 3D viewport: press ++n++ → the **QC Render** tab.
...
| The add-on is listed but `Install` does nothing | Your Blender is older than **5.1.2**. Update Blender |
