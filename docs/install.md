...
Почти все студийные аддоны — это Blender Extensions. Их можно ставить по одному
из файла, но правильный способ один: добавить репозиторий, после чего Blender
сам находит аддоны и предлагает обновления.

Исключение — **QC Bake for Maya**: он живёт в Maya, ставится иначе и обновляет
себя сам. Порядок — [в его обзоре](qc-bake-maya/index.md), ниже коротко.
...
В Maya нет репозитория расширений, поэтому установка своя.

**QC Bake for Maya** — скачать
[`qc_bake_maya.zip`](https://mutaform.github.io/qc-bake-maya/qc_bake_maya.zip),
распаковать туда, где папка останется, и перетащить `install/install.py` во
вьюпорт Maya. Установщик пропишет модуль, добавит кнопку на полку *Mutaform* и
откроет панель. Нужна Maya 2025.

Распакованная папка и есть установка: файл модуля — указатель на неё. Перенесёте
или удалите папку — инструмент отвалится. Обновляется он сам, из панели, без
перезапуска Maya.
