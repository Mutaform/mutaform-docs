...
**Когда пригодится.** Читать всегда, а не только когда что-то пошло не так. `40 cm, 36 → 82` — диаметр, было и стало; `already right` — форма и так правильная, её не трогали; `no cylindrical form` — аддон ничего не узнал и **ничего не испортил**.
...
### Minimum Segments и Even Counts Only

![Minimum Segments и Even Counts Only](img/c/prefs-limits.png){ .control-shot }

Нижняя граница числа сечений и округление до чётного.
...
**По умолчанию:** минимум 6, чётность включена
...
*Страница собрана из файла `content/qc-smart-cylinder.ru.yml`. Снимки сделаны в **QC Smart Cylinder 1.6.0**.*
