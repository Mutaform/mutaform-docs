...
Репозитория аддонов в Maya нет, поэтому установка своя — но в три шага.

1. Скачать [`qc_bake_maya.zip`](https://mutaform.github.io/qc-bake-maya/qc_bake_maya.zip)
2. Распаковать **туда, где папка останется**: в папку инструментов, на сетевую
   шару — куда угодно постоянное
3. Перетащить `install/install.py` из распакованной папки во вьюпорт Maya

Установщик пропишет модуль Maya, добавит кнопку **QC Bake** на полку *Mutaform*
и откроет панель. Дальше Maya подхватывает инструмент при каждом запуске.

Нужна Maya 2025.

!!! warning "Распакованная папка и есть установка"
    Ничего не копируется во внутренние каталоги Maya: файл модуля — это
    указатель на распакованную папку. Поэтому обновление сводится к замене
    одной папки. Обратная сторона: если папку удалить или перенести,
