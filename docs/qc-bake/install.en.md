# Installation — QC Bake

Set it up once; after that Blender offers updates on its own.

## What you need

Blender **5.0** or newer. The version is in the window title, and in
`Help → About Blender`.
...
The panel appears in the 3D viewport: press ++n++ → the **QC Bake** tab.
...
| The add-on is listed but `Install` does nothing | Your Blender is older than **5.0**. Update Blender |
