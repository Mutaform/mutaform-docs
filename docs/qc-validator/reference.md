...
**Когда пригодится.** Переключайте по мере работы. Строгость нарастает слева направо: на Blockout включено 9 проверок, на Textures — 23. Требовать чистую развёртку на блокауте бессмысленно, а перед текстурированием обязательно.
...
!!! warning "Обратите внимание"
    Стадия LP_UVs — особая: только на ней в панели появляются три инструмента осмотра развёртки. На других стадиях их просто нет. Стадия переключает только те проверки, что в ней перечислены; остальные остаются как были. Так, **Nanite Closed Geometry** во встроенный проект не входит — она включена с самого начала, и ни одна стадия её не гасит. Если проект не идёт в Unreal, снимите с неё галочку руками, иначе она будет срабатывать на каждом прогоне.
...
Три интерактивных инструмента: они ничего не пишут в результаты, а показывают состояние прямо в редакторе UV.
...
!!! warning "Обратите внимание"
    Галочка слева важнее, чем кажется. Без неё смотрится только активный объект; с ней — все видимые меши с тем же материалом, то есть всё, что попадёт в один атлас. Два объекта по отдельности могут быть чистыми и налезать друг на друга в общей развёртке — без этой галочки такое не находится. Поле UV set справа — номер набора, считая с единицы.
...
!!! warning "Обратите внимание"
    При смене размера текстуры отступ пересчитывается пропорционально — соотношение сохраняется, и не приходится пересчитывать в уме.
...
Все материалы в текущей области действия: сколько объектов и слотов использует каждый, и кнопка выделения справа.

**Когда пригодится.** Перед запеканием — понять, что попадёт в один атлас. Кнопка справа выделяет все видимые меши с этим материалом.

!!! warning "Обратите внимание"
    Список подчиняется выбранному Scope. С Selection он покажет материалы только выделенных объектов.
...
*Страница собрана из файла `content/qc-validator.ru.yml`. Снимки сделаны в **Scene QC Validator by Mutaform Studio 1.8.7**.*
